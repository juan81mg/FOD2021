...
procedure baja(var empleados: arc_emp);
    var
        n: Integer;
        emp: empleado;
        pos: Integer;
        ultimo_reg: empleado;
    begin
        write('ingrese el numero de empleado a dar de baja --->> ');
        readln(n);

        read(empleados, emp);

        //busco el registro buscado y guardo la posicion
        while (not eof (empleados)) and (n <> emp.num) do begin
        ...
        read(empleados, ultimo_reg);

        //busco la posicion del buscado y guardo el ultimo registro
        seek(empleados, pos);
        write(empleados, ultimo_reg);
        
        //trunco el ultimo registro, marco con eof el ultimo registro
        seek(empleados, FileSize(empleados)-1);
        truncate(empleados);
        
        writeln('El registro dado de bajo es NUMERO', ultimo_reg.num, ' APELLIDO ', ultimo_reg.apellido, ' NOMBRE ', ultimo_reg.nombre, ' EDAD ', ultimo_reg.edad, ' DNI ', ultimo_reg.dni);
...
procedure submenu(var op : integer; var empleados:arc_emp);
	begin
		...
		writeln('');
		writeln('(10) Baja de un empleado');
		writeln('');        
		writeln('(0) Salir');
		writeln('');
		write('Opcion --->> ');
